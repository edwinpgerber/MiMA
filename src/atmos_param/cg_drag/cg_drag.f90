...
real       ::  damp_level_pressure=0.8e+02
				  ! added by cig, feb 27, 2017. Any GW momentum that
                                  ! reaches the model top will be evenly spread over all
                                  ! levels above this pressure level [Pa]
...
!----------------------------------------------------------------------
              if ( k < iz0) then
                ...
                !epg + cig: enforce momentum conservation at the model top. All the momentum to be 
                !     deposited in the uppermost layer (wv_frcng(0), which exist above the top model 
                !     level, as explained in cg_drag_calc) will be evenly spread out to
                !     all layers above "damp_level_pressure" a few lines below.
                if ( k==0 ) then
                   wv_frcng(k+1) =  0.5*wv_frcng(k+1)
                else
              ...
              !cig: place the momentum flux that would escape the model top in the layers above a 
              !     specific threshold (defined by damp_level_pressure).  This corresponds to the 
              !     top iztop levels in the model
 	   do k=1,iztop
